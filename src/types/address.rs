...
impl Address {
    pub fn from_public_key_bytes(bytes: &[u8; 32]) -> Address {
        let clipped_bytes: &[u8; 32] = bytes.try_into().unwrap();
        let bytes_h256: H256 = std::convert::From::from(clipped_bytes);
        let bytes_hash: H256 = bytes_h256.hash();
        let hash_slice: [u8; 32] = std::convert::From::from(&bytes_hash);
        let address_slice: &[u8; 20] = (&hash_slice[12..32]).try_into().unwrap();
        let address: Address = std::convert::From::from(address_slice);
        address
    }
}
